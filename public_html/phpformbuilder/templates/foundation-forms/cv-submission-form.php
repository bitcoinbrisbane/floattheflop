...
$form->addInput('text', 'user-name', '', 'Name', 'class=input-group-field, required');
$form->addIcon('user-first-name', '<i class="input-group-label fi-torso"></i>', 'before');
$form->addInput('text', 'user-first-name', '', 'Firstname', 'class=input-group-field, required');
$form->addIcon('user-email', '<i class="input-group-label fi-mail"></i>', 'before');
$form->addInput('email', 'user-email', '', 'Email', 'class=input-group-field, required');
...
$current_file = '';
if (isset($_POST['cv']) && !empty($_POST['cv']) && !empty($_SESSION['errors']['cv-submission-form'])) {
...
$form->addBtn('submit', 'submit-btn', 1, 'Submit CV', 'class=success button');
$form->endFieldset();
...
<head>
    ...
    <meta name="description" content="Foundation Form Generator - how to create a CV Submission Form with Php Form Builder Class">
